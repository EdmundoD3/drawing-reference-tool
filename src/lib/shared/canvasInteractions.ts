...
export function onPointerMove(e: PointerEvent) {
    ...
    if (inside) {
        ...
        coordsText.x =
            `${fmt(p.x / toolState.pxPerUnit)} ${toolState.unit}`;

        coordsText.y =
            `${fmt(p.y / toolState.pxPerUnit)} ${toolState.unit}`;
...
// Helpers
// ---------------------------------------------------------------
function getCanvasPoint(e: Pick<MouseEvent, "clientX" | "clientY">,): Point & { rect: DOMRect } | null {
